    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className=' w-96 px-5 py-10 rounded-md flex flex-col gap-y-5'>
        ...
        <div className='flex flex-col'>
          ...
          <input {...register("full_name", { required: "Fullname is required" })} type="text" name="full_name" id="full_name" className='rounded-md border p-2' />
          {errors.full_name?.type === "required" && <p role='alert' className='text-xs text-red-500 mt-1'>{errors.full_name.message}</p>}
        </div>
        <div className='flex flex-col'>
        ...
        <div className='flex flex-col'>
          ...
          <input {...register("password", {
            ...
            validate: {
              minCharacters: (v) => v.length >= 8 || "Password minimum 8 characters",
              maxCharacters: (v) => v.length <= 20 || "Password maximum 20 characters",
              includeUppercase: (v) => /[A-Z]/g.test(v) || "Must include uppercase character",
              notIncludeUsername: (v) => !v.includes(getValues("username")) || "Cannot contain username"
            }
          })} type="password" name="password" id="password" className='rounded-md border p-2' maxLength={20} />
